...
public class tmall_SearchAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    boolean isfootview = true;
    private Context context;
    private List<tmall_Product> data;
    private final int N_TYPE=0;
    private final int F_TYPE=1;
    //  预加载的数据的条目
    private int Max_num=15;

    public tmall_SearchAdapter(Context context, List<tmall_Product> data){
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if(viewType==F_TYPE){
            return new tmall_SearchAdapter.RecyclerViewHolder(footview,F_TYPE);
        }else{
            return new tmall_SearchAdapter.RecyclerViewHolder(view,N_TYPE);
        }
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if(isfootview &&getItemViewType(position)==F_TYPE){
            //底部加载
            final tmall_SearchAdapter.RecyclerViewHolder recyclerViewHolder= (tmall_SearchAdapter.RecyclerViewHolder) holder;
...
//              总条目自增8条
            ...
            //获取内容
            final tmall_SearchAdapter.RecyclerViewHolder recyclerViewHolder= (tmall_SearchAdapter.RecyclerViewHolder) holder;
            tmall_Product product=data.get(position);
